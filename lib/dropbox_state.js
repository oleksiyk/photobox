...
var DropboxState = function(dropbox, runtimeDir){
    ...
    this.state = new persistentObject(path.resolve(runtimeDir, '.dropbox.state'), {
        updated: null,
        cursor: null,
        fs: {}
    });
...
DropboxState.prototype.list = function(root, path, white_attrs){
    ...
    if(obj){
        listing = {
            metadata: _.pick(obj.metadata, white_attrs),
            entries: {}
        }

        listing.metadata.path = listing.metadata.path.substring(root.length) || "/";

        _.each(obj.entries, function(value, key){
            listing.entries[key] = {
                metadata: _.pick(value.metadata, white_attrs)
            }
            listing.entries[key].metadata.path = listing.entries[key].metadata.path.substring(root.length);
        })
    }
